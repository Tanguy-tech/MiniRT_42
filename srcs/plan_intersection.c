...
/*   Updated: 2021/03/22 11:18:05 by tbillon          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	plan_intersection(t_scene *rt, t_element *plan, t_vectors *ori, t_vectors *dir)
{
	...
	if (res->t1 < 0)
		return (0);
	rt->t = res->t1;
	// plan->P->x = vec_plus(rt->ray->orig, vec_x(rt->t, rt->ray->norm_dir))->x;
	// plan->P->y = -vec_plus(rt->ray->orig, vec_x(rt->t, rt->ray->norm_dir))->y;
	// plan->P->z = vec_plus(rt->ray->orig, vec_x(rt->t, rt->ray->norm_dir))->z;
	return (1);
}
